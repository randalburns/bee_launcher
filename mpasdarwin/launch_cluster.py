...
def main():
  ...
  parser = argparse.ArgumentParser(description='Launch MPAS in a VM on this darwin node')
  ...
  for node in sorted(result.node):
    ...
    cmd = ["ssh","{}".format(node),"-x",r"ssh -o StrictHostKeyChecking=no -i {}/darwindkr.rsa docker@{} -x 'git clone https://guanxyz:guan5582@github.com/randalburns/mpasdocker.git'".format(os.path.dirname(os.path.realpath(__file__)),nodeip)]
    if result.verbose:
      print cmd
    subprocess.call(cmd)

    cmd = ["ssh","{}".format(node),"-x",r"ssh -o StrictHostKeyChecking=no -i {}/darwindkr.rsa docker@{} -x 'cd /home/docker/mpasdocker; git checkout darwin'".format(os.path.dirname(os.path.realpath(__file__)),nodeip)]
    if result.verbose:
      print cmd
    subprocess.call(cmd)
    ...
    # build container and run sshd on all slave VMs
      ...
      cmd = ["ssh","{}".format(node),"-x",r"ssh -o StrictHostKeyChecking=no -i {}/darwindkr.rsa docker@{} -x /home/docker/mpasdocker/build_vpic.sh".format(os.path.dirname(os.path.realpath(__file__)),nodeip)]
      if result.verbose:
        print cmd
      subprocess.call(cmd)
      ...
      cmd = ["ssh","{}".format(node),"-x",r"ssh -o StrictHostKeyChecking=no -i {}/darwindkr.rsa docker@{} -x 'nohup /home/docker/mpasdocker/run_sshd.sh > /home/docker/mpasdocker/vpicerrlog.out 2>&1 &'".format(os.path.dirname(os.path.realpath(__file__)),nodeip)]
      if result.verbose:
        print cmd
      subprocess.call(cmd)
  ...
  for mpiaddr in mpiaddrs:
    ...
    if len(result.node) == 2:
      ...
      print "Bad MPAS domain decomposition.  Talk to Randal or David."

  # build a machinefile
  cmd = ["ssh","{}".format(masternode),"-x",r"ssh -o StrictHostKeyChecking=no -i {}/darwindkr.rsa docker@{} -x 'cat > /home/docker/mpasdocker/machinefile'".format(os.path.dirname(os.path.realpath(__file__)),masterip)]
  p = subprocess.Popen ( cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)  
  output = p.communicate(input=mfilestr)

  # now build the master container -- must have machinefile first
  cmd = ["ssh","{}".format(masternode),"-x",r"ssh -o StrictHostKeyChecking=no -i {}/darwindkr.rsa docker@{} -x /home/docker/mpasdocker/build_mpas.sh".format(os.path.dirname(os.path.realpath(__file__)),masterip)]
  if result.verbose:
    print cmd
  subprocess.call(cmd)
  ...
  if not result.interactive:
    ...
    cmd = ["ssh","{}".format(masternode),"-x",r"ssh -o StrictHostKeyChecking=no -i {}/darwindkr.rsa docker@{} -x 'nohup /home/docker/mpasdocker/run_mpas.sh > /home/docker/mpasdocker/mpasrunlog.out 2>&1 &'".format(os.path.dirname(os.path.realpath(__file__)),masterip)]
    if result.verbose:
      print cmd
    subprocess.call(cmd)
  else:
