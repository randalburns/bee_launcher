...
def main():
  ...
  for node in sorted(result.node):
    ...
    cmd = ["ssh","{}".format(node),"-x",r"ssh -o StrictHostKeyChecking=no -i {}/darwindkr.rsa docker@{} -x /home/docker/vpicdocker/stop_vpic.sh".format(os.path.dirname(os.path.realpath(__file__)),nodeip)]
    if result.verbose:
      print cmd
    subprocess.call(cmd)
